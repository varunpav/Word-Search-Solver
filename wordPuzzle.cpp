...
int main(int argc, char* argv[]){
  ...
  hashTable hashtable(totlines*8);
  ...
  //output words with length greater than 3
    for (int stROW = 0; stROW<rows; stROW++){
	...
	for (int dir = 0; dir<8; dir++){
	  ...
	  for (int len = 3; len<longestword; len++){
	    string word = getWordInGrid(stROW, stCOL, dir, len, rows, cols);
	    if (word.length()!=len)
	      break;
	    if (word.length()>=3 && hashtable.contains(word)){
	      cout<<dirval<<"("<<stROW<<", "<<stCOL<<"):       "<<word<<endl;
	      totalwords++;
	    }
	  }
	}
    ...
    x.stop();
    cout<<totalwords<<" words found"<<endl;
  return 0;
}
